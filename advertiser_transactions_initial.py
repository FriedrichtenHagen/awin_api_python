...
def loop_api_calls():
    ...
    number_of_necessary_api_calls = math.ceil(total_date_difference/31)
    days_for_last_api_call = total_date_difference % 31
    ...
    for i in range(number_of_necessary_api_calls):
        ...
        if(days_for_last_api_call and i == number_of_necessary_api_calls-1):
            start_date = total_start_date + (i-1)*timedelta(days=31)
            end_date = start_date + timedelta(days=days_for_last_api_call)
            
        else:
            start_date = total_start_date + i*timedelta(days=31)
            end_date = start_date + timedelta(days=31)
        print(start_date, end_date)

        newest_dataframe = fetch_data(start_date, end_date)
        result_df = result_df.append(newest_dataframe, ignore_index=True)
    print(result_df)
